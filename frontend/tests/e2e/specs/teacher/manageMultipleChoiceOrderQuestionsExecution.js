describe('Manage Multiple Choice Questions Walk-through', () => {
    ...
    function validateOrderQuestionFull(
      title,
      content,
      optionPrefix = 'Option ',
      correctIndex1 = 2,
      ...
      validateOrderQuestion(title, content, optionPrefix, correctIndex1, correctIndex2);
    ...
    before(() => {
      ...
      cy.cleanMultipleChoiceQuestionsByName('Cypress Question Order Example');
    ...
    it('Creates a new multiple choice question with order', function() {
      ...
      ).type('Cypress Question Order Example - 01', { force: true });
      cy.get(
        '[data-cy="questionQuestionTextArea"]'
      ).type('Cypress Question Order Example - Content - 01', { force: true });
  
      cy.get('[data-cy="questionIsToOrder"')
        .should('have.length', 1)
        .each(($el, index, $list) => {
          cy.get($el).within($ls => {
            cy.get(`[data-cy="Switch"]`).check({ force: true });
          });
        });
      ...
      .each(($el, index, $list) => {
        cy.get($el).within($ls => {
          if (index === 2) {
            cy.get(`[data-cy="Order${index + 1}"]`).type('2',{ force: true });
          }
          if (index === 3) {
            cy.get(`[data-cy="Order${index + 1}"]`).type('1',{ force: true });
          }
        });
      });
      ...
      cy.get('[data-cy="questionTitleGrid"]')
        .first()
        .should('contain', 'Cypress Question Order Example - 01');
  
      validateOrderQuestionFull(
        'Cypress Question Order Example - 01',
        'Cypress Question Order Example - Content - 01'
      );
    });
  
    it('Can view order question (with button)', function() {
      cy.get('tbody tr')
        .first()
        .within($list => {
          cy.get('button')
    ...
    it('Can view order question (with click)', function() {
      cy.get('[data-cy="questionTitleGrid"]')
        .first()
        .click();
    ...
    it('Can update title to order question (with right-click)', function() {
      ...
      cy.get('[data-cy="createOrEditQuestionDialog"]')
        ...
        .within($list => {
          ...
          cy.get('[data-cy="questionTitleTextArea"]')
            .clear({ force: true })
            .type('Cypress Question Order Example - 01 - Edited', { force: true });
      ...
      cy.get('[data-cy="questionTitleGrid"]')
        .first()
        .should('contain', 'Cypress Question Order Example - 01 - Edited');
  
      validateOrderQuestionFull(
        (title = 'Cypress Question Order Example - 01 - Edited'),
        (content = 'Cypress Question Order Example - Content - 01')
      );
    });

    it('Can update content to order question (with button)', function() {
      cy.route('PUT', '/questions/*').as('updateQuestion');
      ...
      cy.get('[data-cy="createOrEditQuestionDialog"]')
        ...
        .within($list => {
          ...
          cy.get('[data-cy="questionQuestionTextArea"]')
            .clear({ force: true })
            .type('Cypress New Content For Order Question!', { force: true });
      ...
      validateOrderQuestionFull(
        (title = 'Cypress Question Order Example - 01 - Edited'),
        (content = 'Cypress New Content For Order Question!')
      );
    });
    ...
    it('Can duplicate question', function() {
      ...
      cy.get('[data-cy="questionTitleTextArea"]')
        .should('have.value', 'Cypress Question Order Example - 01 - Edited')
        .type('{end} - DUP', { force: true });
      cy.get('[data-cy="questionQuestionTextArea"]').should(
        'have.value',
        'Cypress New Content For Order Question!'
      );
      ...
      cy.get('[data-cy="questionTitleGrid"]')
        .first()
        .should('contain', 'Cypress Question Order Example - 01 - Edited - DUP');
  
      validateOrderQuestionFull(
        'Cypress Question Order Example - 01 - Edited - DUP',
        'Cypress New Content For Order Question!'
      );
    });
